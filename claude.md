...
**在庫判定方式**:
1. **ボタンのテキストで判定（最優先）**
   - 「カートに入れる」 → `in_stock`（在庫あり）
   - 「予約する」 → `pre_order`（予約受付中）
2. **タグで判定**
   - 「予約」タグ → `pre_order`（予約受付中）
   - 「予約終了」タグ → `sold_out`（予約終了）
3. **ページ全体のテキストで判定（フォールバック）**
   - ボタンやタグが見つからない場合、ページ全体から判定
4. **それ以外 → `out_of_stock`（売り切れ）**
5. **判定できない場合 → `unknown`（不明）**
   - エラー発生時に「詳しくは商品ページをご覧ください」のメッセージを表示

**スクレイピング対策**: カート投入などの自動操作は一切行わず、テキスト判定のみで在庫を検出
...
### 在庫判定ロジックの改善（✅完了）
在庫判定ロジックを以下の通り改善しました：

**改善内容**:
- ✅ ボタン要素を複数のセレクタで検索（`button[type="submit"]`, `button.add-to-cart`等）
- ✅ タグ要素を複数のセレクタで検索（`.product-tag`, `.tag`, `.badge`等）
- ✅ ボタン → タグ → ページ全体の順で判定（フォールバック機能）
- ✅ エラー時は`unknown`を返し、「詳しくは商品ページをご覧ください」のメッセージを表示
- ✅ 判定できない場合は`out_of_stock`（売り切れ）とみなす
...
### 完了タスク
- ✅ [#5] 在庫判定ロジックの改善 - ボタン、タグ、フォールバック判定を実装

### 改善案（機能拡張）
- [Phase 2] 現在の監視中在庫をキーワードで確認する機能
- [#6] テストコードの追加
- [#7] Webダッシュボードの作成
