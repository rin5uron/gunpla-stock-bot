...
| コマンド | なにをする？ | 誰に届く？ |
|----------|-------------|-----------|
| `npm run start` | 在庫チェック | 在庫復活時のみ → 友達全員 |
...
実際の在庫状況を確認して、友達全員に手動で配信するテストだよ。
...
**このテストでは**:
- 実際の在庫状況をチェック
- 「これはテスト用の手動配信です。現在の実際の在庫を表示しています。」という文言付きで配信
- **友達全員**にLINE通知が届く（Broadcast）
...
| `npm run test:status` | 友達全員 | Broadcast |

**重要**: `npm run start`（通常の在庫チェック）では、在庫復活時のみ通知が送られます。在庫変化がない場合は通知されません。
